...
import { evaluate, POLICY_VERSION } from "./policy";
...
describe("policy engine", () => {
  it("allows a drift-correcting shift with a fresh qualifying macro event", () => {
    const v = evaluate(ctx(), shift, "p1");
    expect(v.result).toBe("allow");
    expect(v.clauses.every((c) => c.pass === true)).toBe(true);
  });

  it("halts when no macro event qualifies (B4 unmeasurable)", () => {
    const v = evaluate(ctx({ recentMacro: [] }), shift, "p2");
    expect(v.clauses.find((c) => c.id === "B4-event")?.pass).toBeNull();
    expect(v.result).toBe("halt");
  });

  it("halts on stale market data (B7) even with a qualifying event", () => {
    const v = evaluate(ctx({ dataAgeMs: 11 * 60 * 1000 }), shift, "p3");
    expect(v.clauses.find((c) => c.id === "B7-stale")?.pass).toBe(false);
    expect(v.result).toBe("deny");
  });

  it("denies risk-increasing action in drawdown (B2)", () => {
    const b = book({ openValue24h: 8000 }); // book now worth far less than day-open
    const v = evaluate(ctx({ book: b }), shift, "p4");
    expect(v.clauses.find((c) => c.id === "B2-drawdown")?.pass).toBe(false);
    expect(v.result).toBe("deny");
  });

  it("denies shifts inside the open blackout (B5)", () => {
    const t = Date.parse("2026-09-14T14:45:00Z");
    const v = evaluate(ctx({ now: t }), { ...shift, ts: t }, "p5");
    expect(v.clauses.find((c) => c.id === "B5-blackout")?.pass).toBe(false);
    expect(v.result).toBe("deny");
  });

  it("denies hedge shifts that break the band (B3)", () => {
    const b = book({ positions: { BTCUSDT: { qty: 0.05, market: "crypto" } } }); // hedge ~83%
    const v = evaluate(ctx({ book: b }), shift, "p6");
    expect(v.clauses.find((c) => c.id === "B3-hedge-band")?.pass).toBe(false);
    expect(v.result).toBe("deny");
  });

  it("never fabricates prices: unpriced positions shrink sleeves, not pass gates", () => {
    const pxOf = new Map([["RNVDAUSDT", 223.72]]); // BTC unpriced this tick
    const v = evaluate(ctx({ pxOf }), shift, "p7");
